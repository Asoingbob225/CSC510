...
def test_register_invalid_username_format(client):
    ...
    assert response.status_code == 422
    errors = response.json()["detail"]
    assert any("string does not match pattern" in error["msg"].lower() for error in errors)
...
def test_password_validation(client):
    ...
    test_cases = [
        ...
        },
        {
            ...
            "detail": "string should have at most 48 characters"
        }
    ]
    ...
    for test_case in test_cases:
        ...
        assert test_case["detail"].lower() in [msg.lower() for msg in error_messages]
