...
def test_register_invalid_username_format(client):
    ...
    assert response.status_code == 422
    errors = response.json()["detail"]
    assert any("letters, numbers, underscores" in error["msg"].lower() for error in errors)
...
def test_password_validation(client):
    ...
    test_cases = [
        {
            ...
            "detail": "must be between 8 and 48 characters"
        },
        {
            "password": "nouppercase123!",  # Missing uppercase
            "detail": "must contain at least one uppercase letter"
        },
        {
            "password": "NOLOWERCASE123!",  # Missing lowercase
            "detail": "must contain at least one lowercase letter"
        },
        {
            "password": "NoSpecialChar123",  # Missing special character
            "detail": "must contain at least one special character"
        },
        {
            "password": "NoNumber@abcABC",  # Missing number
            "detail": "must contain at least one number"
        },
        {
    ...
    for test_case in test_cases:
        ...
        assert any(test_case["detail"] in msg for msg in error_messages)
